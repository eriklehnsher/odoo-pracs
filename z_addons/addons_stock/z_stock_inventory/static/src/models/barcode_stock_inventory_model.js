...
export default class BarcodeStockInventoryModel extends BarcodeModel {
    constructor(resModel, resId, services) {
        super(resModel, resId, services)
        this.lineModel = 'stock.quant'
        // this.formViewReference = 'z_stock_inventory.stock_inventory_barcode_view_info'
        this.validateMessage = _t("The Stock Inventory has been validated")
        this.validateMethod = 'action_confirm'
    }
    ...
    get barcodeInfo() {
        ...
        if (this.record.state === 'done') {
            barcodeInfo.class = 'stock_inventory_done'
            barcodeInfo.message = _t('This inventory sheet is complete.')
            barcodeInfo.icon = 'exclamation-triangle'
            return barcodeInfo
        }
    ...
    setData(data) {
        ...
        this.record = this.cache.getRecord(this.resModel, this.resId)
    }
    ...
    getActionRefresh(newId) {
        ...
        if (this.selectedLineVirtualId || newId) {
            action.params.model = "stock.quant"
        }
        action.params.res_id = this.currentState.lines.map(l => l.id)
        if (newId) {
            action.params.res_id.push(newId)
        }
        Object.assign(action.params, {'stock_inventory_id': this.resId})
        return action
    }
    ...
    _createLinesState() {
        ...
        return this.barcodeInfo.class === 'stock_inventory_done' ? [] : lines
    }
    ...
    get canBeProcessed() {
        return this.barcodeInfo.class === 'stock_inventory_done' ? false : true
    }
